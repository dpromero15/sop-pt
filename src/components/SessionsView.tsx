...
export const SessionsView: React.FC<SessionsViewProps> = ({
  ...
  return (
    <div className="space-y-4 pb-28">
      ...
      {/* Sessions list: newest first, expand in place */}
      <div className="space-y-1">
          ...
          {filteredSessions.map(session => {
            ...
            const sessionEntries = isSelected
            ...
            return (
              <div
                ...
                className={`border overflow-hidden transition-all duration-200 ${
                  isSelected
                    ? 'bg-slate-800/80 border-purple-500/60 rounded-xl'
                    : 'bg-slate-900/80 hover:bg-slate-800/60 border-slate-800 rounded-xl'
                }`}
              >
                <div
                  role="button"
                ...
                <ChevronDown
                  className={`w-3.5 h-3.5 shrink-0 text-slate-500 transition-transform ${
                    isSelected ? 'rotate-180 text-purple-300' : ''
                  }`}
                />
                ...
                {isSelected && selectedSession && (
                  <div
                    className="border-t border-slate-700/80 bg-slate-950/50 px-3 py-3 space-y-3"
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                      ...
                      <button
                        onClick={(e) => {
                        e.stopPropagation();
                        onOpenQuickInsertForSession(selectedSession.id);
                      }}
                        className="flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-extrabold text-xs transition-all active:scale-95 shadow-lg shadow-emerald-500/20 shrink-0"
                      >
                        <Plus className="w-4 h-4" />
                        <span>
                    ...
                    <div className="space-y-2">
                      <h4 className="text-[10px] font-bold uppercase text-slate-500 tracking-wider">
                        Logged ({sessionEntries.length})
                      </h4>

                      {sessionEntries.length === 0 ? (
                        <div className="bg-slate-950 border border-slate-800/80 rounded-lg px-3 py-3 text-center space-y-1">
                        ...
                        <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1">
                          {players.map(player => {
                            const pEntries = sessionEntries.filter(e => e.playerId === player.id);
                            if (pEntries.length === 0) return null;

                            return (
                              <div
                                key={player.id}
                                className="bg-slate-950 border border-slate-800 rounded-lg px-2.5 py-2 flex flex-col sm:flex-row sm:items-center justify-between gap-1.5"
                              >
